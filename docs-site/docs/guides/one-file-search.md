...
The complete `.pikelet` profile has five segment types:
...
- `lexical` (optional): a BM25 term/postings index used for hybrid retrieval (the default) and `retrieval: 'lexical'`; readers that predate it skip the segment.
