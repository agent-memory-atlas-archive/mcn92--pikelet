...
/**
 * Pikelet vs USearch Benchmark (DBpedia 100K, L2, 1536D)
 *
 ...
 *
 * Usage:
 *   node benchmarks/engine/benchmark_usearch.js
 *   node benchmarks/engine/benchmark_usearch.js --regenerate-gt
 *   node benchmarks/engine/benchmark_usearch.js --m 16 --ef-construction 100
 */
...
async function sweepOne(config, dataset) {
  ...
  for (const efSearch of EF_SEARCH_VALUES) {
    ...
    for (let rep = 0; rep < REPETITIONS; rep++) {
      let result;
      if (config.library === 'pikelet') {
        result = queryPikelet(index, test, groundTruth, efSearch);
      } else if (config.library === 'usearch') {
...
async function main() {
  ...
  log('='.repeat(70));
  log('Pikelet vs USearch Benchmark (DBpedia 100K, L2, 1536D)');
  log('='.repeat(70));
