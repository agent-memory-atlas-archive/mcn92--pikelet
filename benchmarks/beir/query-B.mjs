...
//
// Usage: node benchmarks/beir/query-B.mjs <dataset>
...
if (!dataset) {
  console.error('usage: node benchmarks/beir/query-B.mjs <dataset>');
  process.exit(1);
}
...
for (const p of [corpusVecPath, corpusIdsPath, queryVecPath, queryIdsPath]) {
  if (!existsSync(p)) {
    console.error(`missing ${p} — run: node benchmarks/beir/query-C.mjs ${dataset} first (it produces and caches the Pikelet-encoder vectors)`);
    process.exit(1);
  }
}
