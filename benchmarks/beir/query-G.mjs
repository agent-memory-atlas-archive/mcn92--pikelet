...
//
// Usage: node benchmarks/beir/query-G.mjs <dataset> [--no-prefix]
...
if (!dataset) {
  console.error('usage: node benchmarks/beir/query-G.mjs <dataset> [--no-prefix]');
  process.exit(1);
}
...
for (const p of [corpusVecPath, corpusIdsPath, queryVecPath, queryIdsPath]) {
  if (!existsSync(p)) {
    console.error(`missing ${p} — run: node benchmarks/beir/encode-pikelet-arctic.mjs ${dataset} ${noPrefix ? '--no-prefix' : ''} first`);
    process.exit(1);
  }
}
