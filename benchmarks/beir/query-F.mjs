...
//
// Usage: node benchmarks/beir/query-F.mjs <dataset>
...
if (!dataset) {
  console.error('usage: node benchmarks/beir/query-F.mjs <dataset>');
  process.exit(1);
}
...
for (const p of [corpusIdsPath, recordsPath]) {
  if (!existsSync(p)) {
    console.error(`missing ${p} — run: node benchmarks/beir/embed.py ${dataset} corpus first (id map only, no vectors needed)`);
    process.exit(1);
  }
}
