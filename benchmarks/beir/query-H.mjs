...
//
// Usage: node benchmarks/beir/query-H.mjs <dataset>
...
if (!dataset) {
  console.error('usage: node benchmarks/beir/query-H.mjs <dataset>');
  process.exit(1);
}
...
for (const p of [corpusVecPath, corpusIdsPath, queryVecPath, queryIdsPath, recordsPath]) {
  if (!existsSync(p)) {
    console.error(`missing ${p} — run: node benchmarks/beir/encode-pikelet-arctic.mjs ${dataset} first`);
    process.exit(1);
  }
}
