#!/usr/bin/env node
// Convert a downloaded BEIR dataset (benchmarks/beir/cache/<dataset>/) into Pikelet
// source records (benchmarks/beir/work/<dataset>/records.jsonl) plus an id map
// (benchmarks/beir/work/<dataset>/id-map.json) from Pikelet row index -> BEIR id.
//
...
//
// Usage: node benchmarks/beir/convert.mjs <dataset>
...
if (!dataset) {
  console.error('usage: node benchmarks/beir/convert.mjs <dataset>');
  process.exit(1);
}
...
if (!existsSync(cacheDir)) {
  console.error(`${cacheDir} not found — run: python3.11 benchmarks/beir/download.py ${dataset}`);
  process.exit(1);
}
