...
//
// Usage: node benchmarks/beir/ablate-query-prefix.mjs <dataset>
...
if (!dataset) {
  console.error('usage: node benchmarks/beir/ablate-query-prefix.mjs <dataset>');
  process.exit(1);
}
...
if (!existsSync(path.join(workDir, 'vectors-corpus-arctic.f32'))) {
  console.error(`missing corpus vectors — run: node benchmarks/beir/encode-pikelet-arctic.mjs ${dataset} first`);
  process.exit(1);
}
