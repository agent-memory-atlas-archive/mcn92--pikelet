...
//
// Usage: node benchmarks/beir/query-A.mjs <dataset>
...
if (!dataset) {
  console.error('usage: node benchmarks/beir/query-A.mjs <dataset>');
  process.exit(1);
}
...
function loadVectors(prefix) {
  ...
  if (!existsSync(idsPath) || !existsSync(vecPath)) {
    console.error(`missing ${idsPath} or ${vecPath} — run: python3.11 benchmarks/beir/embed.py ${dataset} ${prefix}`);
    process.exit(1);
  }
